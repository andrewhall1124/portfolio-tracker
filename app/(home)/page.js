    ...
    try {
      ...
      if (status >= 200) {
        const portfolio = data.reduce((acc, order) => {
          const { ticker, num_shares, purchase_price, beta } = order;
        ...
        for (const ticker in portfolio) {
          ...
          portfolio[ticker].average_cost =
            Math.round((purchase_price / num_shares) * 100) / 100;
        }

        console.log(portfolio);

        const oldestDate = data.reduce((oldest, current) =>
          current.purchase_date < oldest.purchase_date ? current : oldest
        ).purchase_date;
        ...
        for (const ticker in portfolio) {
          ...
          currentTicker.return_td =
            Math.round((curr / currentTicker.average_cost - 1) * 10000) / 100;
        }

        const portfolioArray = Object.entries(portfolio).map(
          ([ticker, data]) => ({
            ticker,
